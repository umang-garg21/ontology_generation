...
from BQ_run_module import run_module
...
class PythonScriptWrapper(object):
    def __init__(self):
        for file in (
            os.listdir()
        ):  # Might change it to read parameters from .JSON or from modulePath variable
            if file.endswith(".xml"):
                # Get xml file name as module name
                if hasattr(self, "module_name"):
                    raise ScriptError(
                        "More than 1 .xml file present in directory, make appropiate changes and rebuild image"
                    )
                else:
    ...
    def upload_results(self, bq):
        ...
        for resource in nonimage_tag.findall(".//*[@type]") + outputs_tag.findall(
            "./*[@type='image']"
        ):
            ...
            log.info(
                "***** Uploading output %s '%s' from %s ..."
                % (resource_type, resource_name, resource_path)
            )

            # Upload output resource to Bisque and get resource etree.Element
            output_etree_Element = self.upload_service(
                bq, resource_path, data_type=resource_type
            )
            # log.info(f"***** Uploaded output {resource_type} '{resource_name}' to {output_etree_Element.get('value')}")
            log.info(
                "***** Uploaded output %s '%s' to %s"
        ...
        log.debug(
            "***** Output Resources xml : output_resources = %s" % output_resources
        )
        # SAMPLE LOG
    ...
    def fetch_input_resources(
        self, bq, inputs_dir_path
    ):  # TODO Not hardcoded resource_url
        """
        ...
        for input_resource in inputs_tag.findall("./*[@type='resource']"):
            ...
            input_path_dict[input_name] = os.path.join(
                inputs_dir_path, resource_obj.name
            )

            # Saves resource to module container at specified dest path
            fetch_blob_output = fetch_blob(
                bq, resource_obj.uri, dest=input_path_dict[input_name]
            )
    ...
    def run(self):
        ...
        try:
            ...
            bq.fail_mex(
                msg="Exception while fetching inputs specified in xml: %s" % str(e)
            )
            return
        ...
        try:
            ...
            self.output_data_path_dict = run_module(
                input_path_dict, outputs_dir_path, bq
            )
        except (Exception, ScriptError) as e:
            log.exception("***** Exception while running module from BQ_run_module")
            bq.fail_mex(
                msg="Exception while running module from BQ_run_module: %s" % str(e)
            )
            return
    ...
    def tear_down(self):  # NEED TO GENERALIZE
        ...
        for r_xml in self.output_resources:
            ...
            res_type = (
                r_xml.get("type", None) or r_xml.get("resource_type", None) or r_xml.tag
            )
        ...
        log.debug(
            "Output Mex results: %s" % (etree.tostring(outputTag, pretty_print=True))
        )
    ...
    def upload_service(self, bq, filename, data_type="image"):
        ...
        if r is None or r.get("uri") is None:
            ...
            log.info(
                "Uploaded ID: %s, URL: %s" % (r.get("resource_uniq"), r.get("uri"))
            )
            bq.update_mex(
                "Uploaded ID: %s, URL: %s" % (r.get("resource_uniq"), r.get("uri"))
            )
    ...
    def validate_input(self):
        ...
        """
        if (
            self.options.mexURL and self.options.token
        ):  # run module through engine service
            return True
        if (
            self.options.user and self.options.pwd and self.options.root
    ...
    def main(self):
        ...
        if self.validate_input():
            ...
            # initalizes if mex and mex token is provided
                ...
                try:
                    self.bqSession = BQSession().init_mex(
                        self.options.mexURL, self.options.token
                    )
                except:
                    return
